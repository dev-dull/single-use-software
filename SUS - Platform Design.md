...
**Proxy targets are resolved server-side.** The build and run proxy routes derive the destination pod from the caller's authenticated session (`SessionStore`), never from a client-supplied `pod_ip`. This closes the cross-pod-takeover / in-cluster SSRF vector (#80 — a client could otherwise proxy to any pod or host) and keeps the address current across pod churn. The browser addresses its session only through `/build/{team}/{app}/…` routes; readiness is exposed via a `/status` endpoint that never returns a pod IP.
